...
package dev.dejay.ezlib.components.configuration;

import dev.dejay.ezlib.core.EZComponent;
import dev.dejay.ezlib.core.EZLib;
import dev.dejay.ezlib.tools.Callback;
...
public class ConfigurationInstance extends EZComponent {
     ...
     * Creates a configuration instance from a file.
     *
     * @param fileName The name, relative to the plugin's data folder, of the file containing the
     *                 YAML configuration.
     */
     ...
     *
     * @throws IOException                   If the configuration is loaded from a file and this
     ...
     *
     * @throws IllegalStateException If the configuration was directly constructed from a
     *                               {@link FileConfiguration}, as there is nothing to reload the
     *                               configuration from.
     */
     ...
     *                        {@link FileConfiguration} directly, we assume that we have to reload
     *                        the main plugin configuration from config.yml, as this configuration
     *                        is loaded directly. This option is used internally in
     *                        {@link Configuration} for the reload method.
     * @throws IllegalStateException If the configuration was directly constructed from a
     *                               {@link FileConfiguration}, and {@code assumeConfigYml} is
     *                               false, as there is nothing to reload the configuration from.
     */
    void reload(boolean assumeConfigYml) throws IllegalStateException {
        try {
            ...
            if (underlyingFile != null) {
                ...
                EZLib.getPlugin().reloadConfig();

                // The instance change when the reloadConfig method is called.
                this.bukkitConfiguration = EZLib.getPlugin().getConfig();
            } else {
     ...
     *
     * @return The file path, or {@code null} if no file name is set. This may return a relative
     * path to the plugin's data folder.
     * @see #getUnderlyingFile() Method to get a {@code File} object directly, without path
     * problems.
     */
     ...
     *
     * @return A {@link File} object pointing to the underlying file, or {@code null} if no file
     * name is set. This assumes that the file name is relative to the plugin's data folder.
     */
    private File getUnderlyingFile() {
        if (file == null && fileName == null) {
            return null;
        }
        return file != null ? file : new File(EZLib.getPlugin().getDataFolder().getAbsolutePath() + "/" + fileName);
    }
     ...
     *
     * @throws IllegalStateException If the configuration was directly constructed from a
     *                               {@link FileConfiguration}, as there is nothing to save the
     *                               configuration to.
     */
     ...
     *
     * @param assumeConfigYml If {@code true}, and the configuration was loaded from a
     *                        {@link FileConfiguration} directly, we assume that we have to save the
     *                        main plugin configuration to config.yml, as this configuration is
     *                        loaded directly. This option is used internally in
     *                        {@link Configuration} for the save method.
     * @throws IllegalStateException If the configuration was directly constructed from a
     *                               {@link FileConfiguration}, and {@code assumeConfigYml} is
     *                               false, as there is nothing to save the configuration to.
     */
